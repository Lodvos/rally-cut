...
![Найденные розыгрыши](docs/screenshot-rallies.png)

*Матч на 13 минут: 68 розыгрышей, половина видео — пауза между очками.
Жёлтым помечены места, которые стоит проверить.*
...
| **С распознаванием поз** | **92%** | **96%** |

Границы попадают с точностью около 0.5 секунды. Анализ часового матча занимает
примерно 11 минут на MacBook с M3.
...
> После обновления кода сервер нужно перезапустить: интерфейс подхватывается
> сразу, а серверная часть — только при старте. Если они разошлись, наверху
...
Вкладка **«Разбор»** — для работы над ошибками. Нажать «идти по розыгрышам»:
они проигрываются подряд, под видео открывается поле для заметки и быстрые метки
(ошибка подачи, приём, топспин, в сетку, за стол и другие). Написали —
**Enter** включает следующий розыгрыш.

Кнопка **«Собрать разбор»** сохраняет всё в `output/<видео>_разбор.md`: таблица
с таймкодами, длительностью, счётом на момент розыгрыша, победителем, метками
и вашими комментариями. Такой файл удобно отдать на анализ — человеку или
языковой модели.
...
```
2026-02-06_1_0-3.mov            матч, счёт 0:3
2026-08-29_2_1-3.mov            матч, счёт 1:3
2026-08-29_2_1-3_short.mov      ручная нарезка того же матча
2026-09-12_1.mov                матч
2026-09-12_тренировка_1.mov     запись тренировки
```

Тренировки нумеруются отдельно от матчей — иначе добавленная запись сдвигала бы
номера уже разобранных матчей того же дня.
